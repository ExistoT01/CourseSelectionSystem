...
public class AdminController {
    ...
    public Result deleteStudentByName(@RequestParam("name") String name) {
        //解决中文乱码   要注意用SpringBoot时不需要进行中文乱码处理
        byte[] bytes = name.getBytes(StandardCharsets.ISO_8859_1);
        name = new String(bytes,StandardCharsets.UTF_8);

        boolean res = studentService.deleteStudentByName(name);
        return new Result(res ? Code.DEL_OK : Code.DEL_ERR, res);
    }
    ...
    public Result modifyStudent(@RequestBody Student student) {
        System.out.println("参数传递 ==> " + student);
        boolean res = studentService.modifyStudent(student);
        return new Result(res ? Code.ADD_OK : Code.ADD_ERR, res);
    }
